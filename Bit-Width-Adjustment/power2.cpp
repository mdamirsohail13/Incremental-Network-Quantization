...
namespace caffe
{
  ...
  double weightCluster( Dtype weight, int M)
  {
    ...
    template <typename Dtype>
  double weightCluster_zero( Dtype weight, int M) // M is actually n1 (Amir)
  {
    double min=100;
    double ind=0;
    double flag=1.0;
    ...
    for(int i=(M-7);i<=M;i++) // finding nearest power of 2 (Amir)
      {
        if(min>std::abs(weight-pow(2,i))) // [2^(n1-7) to 2^n1] (Amir)
          {
            min=std::abs(weight-pow(2,i));
            ind=i;
            flag=1.0;
